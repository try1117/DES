...
vector<int48> GetKeys(int64 key)
{
	...
	unsigned long long cc = C.to_ullong();
	unsigned long long dd = D.to_ullong();
	...
	for (int i = 0; i < 16; ++i) {
		...
		//int56 CD = MergeNumbers<int28, int56>(vector<int28>({ D, C }), 2, 28);
		res.push_back(ApplyPermutation<int56, int48>(CD, PC_2));

		unsigned long long cc = C.to_ullong();
		unsigned long long dd = D.to_ullong();
		unsigned long long cdcd = CD.to_ullong();
		cdcd = cdcd;

		unsigned long long klu = res.back().to_ullong();
		klu = klu;
	}
	return res;
}
...
int32 f(int32 R, int48 K)
{
	...
	unsigned long long erk = (E(R) ^ K).to_ullong();
	vector<int6> B = SplitNumber<int48, int6>(E(R) ^ K, 8, 6);
	vector<int4> S(8);

	vector<unsigned long long> ss(8);
...
int64 Encrypt_Decrypt(int64 arg, int64 key, bool to_decrypt = false)
{
	...
	unsigned long long tmp_key = key.to_ullong();

	vector<int48> K = GetKeys(key);
	if (to_decrypt)	reverse(K.begin(), K.end());

	vector<unsigned long long> tmp;
	for (auto i : K) tmp.push_back(i.to_ullong());

	for (int i = 0; i < 16; ++i) {

		unsigned long long rr = R.to_ullong();
		unsigned long long ll = L.to_ullong();
	...
	ull argarg = arg.to_ullong();

	ull resresres = ApplyPermutation<int64, int64>(arg, reverseInitPerm).to_ullong();
...
void EncryptFile(EncryptionType et, int64 key, int64 initialVector, std::string inputFile, std::string outputFile)
{
	...
	for (idx = 0; idx + 8 <= length; idx += 8) {
		...
		for (int i = 0; i < 8; ++i) bytes[i] = vec[i].to_ulong();
...
void DecryptFile(EncryptionType et, int64 key, int64 initialVector, std::string inputFile, std::string outputFile)
{
	...
	for (idx = 0; idx + 8 <= length; idx += 8) {
		char bytes[8];
		input.read(bytes, 8);
		int64 cur = MergeNumbers<int8, int64>(vector<int8>(bytes, bytes + 8), 8, 8).to_ullong();
		
		unsigned long long curll = cur.to_ullong();
		nextValues(key, cur, prev_cipher);
		unsigned long long next_curll = cur.to_ullong();
...
bool TestEncryption(EncryptionType et, int64 key, int64 initialVector, std::string inputFileName)
{
	...
	EqualFiles(inputFileName, decFileName);
	return true;
}
